...
class EfficientNetLite(nn.Module):
    def __init__(self, widthi_multiplier, depth_multiplier, num_classes, drop_connect_rate, dropout_rate):
        ...
        mb_block_settings = [
            ...
            [1, 3, 1, 1, 32,  16,  0.25],  # 0
            [2, 3, 2, 6, 16,  24,  0.25],  # 1
            [2, 5, 2, 6, 24,  40,  0.25],  # 2
            [3, 3, 2, 6, 40,  80,  0.25],  # 3
            [3, 5, 1, 6, 80,  112, 0.25],  # 4
            [4, 5, 2, 6, 112, 192, 0.25],  # 5
            [1, 3, 1, 6, 192, 320, 0.25]   # 6
        ]
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.blocks):
            for block in stage:
                ...
                idx +=1
            if i in [1, 2, 3, 6]:  # 1 24,24,16,16 /2 24,40,8,8/3 24,80,4,4/6 24,320,42,2
                features.append(x)
