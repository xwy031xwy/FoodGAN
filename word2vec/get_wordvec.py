...
def get_title_wordvec(recipe, w2i, max_len=18):
    '''
...
def get_instructions_wordvec(recipe, w2i, max_len=20):
    ...
    vec = np.zeros([max_len, max_len], dtype=int)
    num_insts = min(max_len, len(instructions))
    num_words_each_inst = np.zeros(max_len, dtype=int)
...
def get_ingredients_wordvec(recipe, w2i, permute_ingrs=False, max_len=18):
    '''
...
def get_ingredients_wordvec_withClasses(recipe, w2i, ingr2i, permute_ingrs=False, max_len=18):
    '''
    get the ingredients wordvec for the recipe, the
    number of items might be different for different
    recipe
